...
export type ExercisesProps = {
  ...
  queryFunction: ({filters, lowerLimit, upperLimit, guildId}:{filters: TExerciseFilter, lowerLimit?: number, upperLimit?: number, guildId?: string}) => Promise<any>;
  guildId?: string;
};

export const Exercises: React.FC<ExercisesProps> = ({ title, defaultDuration, type, queryFunction, guildId}) => {
  ...
  const {exercises, isLoading, isFetching} = useGetExercisesPaginated(pageInfo, filters, queryFunction, type, guildId);

  const handleChecked = (
    type: "blogArticle" | "studyExercise" | "til"
  ) => {
