...
import { Task } from "common/components/outline/TaskDeprecated";
...
type Props = {
  sectionIndex: number;
  section: TSectionV2;
  checkingDisabled: boolean; // This will be gone
  blogArticle: BlogArticle; // This will be gone
...
export const Section: React.FC<Props> = ({
  ...
  const textAreaRef = useRef<HTMLTextAreaElement>(null); // Make this input instead of textarea
  const [sectionTitle, setSectionTitle] = useState(section?.title || ""); // call it sectionText
  ...
  const handleDelete = (e: any) => {
    ...
    blogArticle.deleteSection(sectionIndex);
  ...
  const handleAddTask = (e: any) => {
    ...
    blogArticle.addTask(sectionIndex, {
      text: "",
      is_action_item: true,
    });
    setBlogArticleXml(blogArticle.blogArticleXml);
