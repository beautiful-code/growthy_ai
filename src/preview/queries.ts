...
export const getExercisePublication = async (exerciseId: string) => {
    ...
    const allTaskIds = sections.flatMap(section => section.getUITasks().map(task => task.getUUID()));
    ...
    const publicationSections: PublicationSection[] = sections.map(section => {
        ...
        return {
            sectionTitle: section.getSectionName(),
            content: sectionContent.map(content => content.data)
        }
    })

    return { publicationTitle, publicationSections } as ExercisePublication 
}
